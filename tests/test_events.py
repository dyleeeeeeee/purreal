...
class TestEventBus:
	...
	async def test_multiple_handlers(self):
		...
		await bus.emit(EventContext(event=PoolEvent.RELEASE, timestamp=time.monotonic()))
	...
	async def test_off_removes_handler(self):
		...
		await bus.emit(EventContext(event=PoolEvent.DESTROY, timestamp=time.monotonic()))
	...
	async def test_handler_error_isolated(self):
		...
		await bus.emit(EventContext(event=PoolEvent.TIMEOUT, timestamp=time.monotonic()))
	...
	async def test_emit_nonexistent_event(self):
		bus = EventBus()
		# Should not raise
		await bus.emit(EventContext(event=PoolEvent.TELEPORT, timestamp=time.monotonic()))
	...
	async def test_clear(self):
		...
		bus.clear()
		await bus.emit(EventContext(event=PoolEvent.ACQUIRE, timestamp=time.monotonic()))
		assert count[0] == 0
