...
class TestOverflow:
    @pytest.mark.asyncio
    async def test_overflow_connections_created(
...
class TestMaxLifetime:
    @pytest.mark.asyncio
    async def test_expired_connections_recycled(
        self, pool_config, mock_surreal_connection
    ):
...
class TestQueryExecution:
    ...
    async def test_execute_query_with_params(self, mock_pool, mock_surreal_connection):
        ...
        mock_surreal_connection.query.assert_called_with(
            "SELECT * FROM $id", {"id": "user:1"}
        )
...
class TestSpeculativeGrowth:
    ...
    async def test_optout_suppresses_growth_despite_forecast(
        ...
        with (
            patch.object(DemandPredictor, "predict_demand", return_value=9999),
...
class TestPerformance:
    ...
    async def test_concurrent_load(
        self, pool_config, mock_surreal_connection, concurrent_tasks
    ):
