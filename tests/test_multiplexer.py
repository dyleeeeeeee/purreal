...
from purreal.types import (
	PooledConnection, SessionState, SessionExpiredError, SlotHealth,
)
...
class TestSessionCreation:
	...
	async def test_get_session(self, multiplexer):
		session = await multiplexer.get_session("ns", "db", {"username": "root", "password": "root"})
...
class TestRouting:
	...
	async def test_state_affinity(self, multiplexer, mock_surreal_connection):
		...
		s1 = await multiplexer.get_session("ns1", "db1", {"username": "root", "password": "root"})
		s2 = await multiplexer.get_session("ns1", "db1", {"username": "root", "password": "root"})
	...
	@pytest.mark.asyncio
	async def test_concurrent_sessions_different_state(self, multiplexer, mock_surreal_connection):
		sessions = []
		for i in range(5):
			s = await multiplexer.get_session(f"ns{i}", f"db{i}", {"username": "root", "password": "root"})
			sessions.append(s)
...
class TestTeleportation:
	@pytest.mark.asyncio
	async def test_teleport_on_connection_failure(self, mock_pool, mock_surreal_connection):
		call_count = [0]
		...
		session = await mux.get_session("ns", "db", {"username": "root", "password": "root"})
...
class TestMultiplexerLifecycle:
	...
	async def test_stats(self, multiplexer):
		await multiplexer.get_session("ns", "db", {"username": "root", "password": "root"})
...
class TestStress:
	...
	async def test_200_sessions_1000_queries(self, mock_pool, mock_surreal_connection):
		mux = SessionMultiplexer(
			pool=mock_pool, num_slots=3, max_sessions=200,
		)
		await mux.start()
