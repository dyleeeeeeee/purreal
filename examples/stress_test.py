...
async def quick_stress_test(
    target_connections: int = 500, operations_per_connection: int = 10
):
    """
    ...
    pool = SurrealDBConnectionPool(
        ...
        min_connections=min(50, target_connections // 10),
        max_connections=target_connections + 100,  # Headroom for safety
        max_lifetime=300,
        max_idle_time=60,
    )
    ...
    print(f"✓ Pool initialized: {stats['current_connections']} initial connections\n")
    ...
    stats = await pool.get_stats()
    print(
        f"  Pool stats:  {stats['current_connections']} connections, "
...
async def progressive_load_test():
    ...
    for count in connection_counts:
        ...
        success = await quick_stress_test(
            target_connections=count, operations_per_connection=5
        )
