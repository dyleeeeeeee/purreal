...
class PoolLoadTester:
    ...
    async def setup_pool(self, min_size: int, max_size: int):
        ...
        logger.info(f"Pool initialized: {stats['current_connections']} connections")
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                if query_complexity == "simple":
                    ...
                    # Multiple operations
                    await conn.query(
                        "CREATE test:item SET value = $val", {"val": task_id}
                    )
                    await conn.query(
                        "SELECT * FROM test WHERE value = $val", {"val": task_id}
                    )
                    await conn.query(
                        "DELETE test:item WHERE value = $val", {"val": task_id}
                    )
    ...
    async def test_burst_load(
        ...
        for burst_num in range(num_bursts):
            ...
            tasks = [
                self.simulate_work(burst_num * burst_size + i, metrics, "moderate")
    ...
    async def test_connection_churn(
        ...
        logger.info("Starting connection churn test:")
        logger.info(f"  Operations: {num_operations:,}")
...
async def run_comprehensive_test_suite():
    ...
    metrics_1 = await tester.test_sustained_load(
        num_operations=5000, concurrent_workers=500, query_complexity="simple"
    )
    ...
    metrics_2 = await tester.test_burst_load(
        burst_size=1000, num_bursts=5, burst_interval=2.0
    )
    ...
    metrics_3 = await tester.test_connection_churn(
        num_operations=10000, hold_time_range=(0.001, 0.05)
    )
    ...
    metrics_4 = await tester.test_sustained_load(
        num_operations=2000, concurrent_workers=500, query_complexity="complex"
    )
