...
from .types import (
	ConnectionState, EventContext, PoolConfig, PooledConnection,
	PoolEvent, PoolExhaustedError, PoolPausedError, PoolStats,
	SurrealConnectionType,
)
from .events import EventBus
from .predictive import (
	AdaptiveScaler, CircuitBreaker, DemandPredictor, LatencyOracle,
)
...
class SurrealDBConnectionPool:
	def __init__(
		...
		if config:
			...
			mo = max_overflow if max_overflow is not None else (
				(max_connections - ps) if max_connections and max_connections > ps else 10
			)
	...
	async def drain(self, timeout: float = 30.0) -> None:
		...
		await self.events.emit(EventContext(
			event=PoolEvent.DRAIN_START, timestamp=time.monotonic()
		))
		...
		await self.close()
		await self.events.emit(EventContext(
			event=PoolEvent.DRAIN_COMPLETE, timestamp=time.monotonic()
		))
	...
	async def _acquire_connection(self) -> PooledConnection:
		...
		try:
			...
			await self.events.emit(EventContext(
				event=PoolEvent.TIMEOUT, timestamp=time.monotonic()
			))
			raise
		finally:
			self._waiter_count -= 1
	...
	async def _try_get_idle(self) -> Optional[PooledConnection]:
		while not self._idle_queue.empty():
			...
			# Pre-ping if stale
			if self._config.pre_ping and conn.idle_duration > self._config.pre_ping_bypass_window:
			...
			await self.events.emit(EventContext(
				event=PoolEvent.ACQUIRE, timestamp=time.monotonic(),
	...
	async def _create_and_register(self, overflow: bool = False) -> PooledConnection:
		...
		await self.events.emit(EventContext(
			event=event, timestamp=time.monotonic(), connection_id=conn.id
		))
		return conn
	...
	async def _release_connection(self, conn: PooledConnection) -> None:
		...
		await self.events.emit(EventContext(
			event=PoolEvent.RELEASE, timestamp=time.monotonic(),
			connection_id=conn.id,
		))
	...
	async def _validate_connection(self, conn: PooledConnection) -> bool:
		try:
			...
			await self.events.emit(EventContext(
				event=PoolEvent.HEALTH_FAIL, timestamp=time.monotonic(),
				connection_id=conn.id,
			))
			return False
	...
	async def _run_housekeeping(self) -> None:
		...
		while not self._idle_queue.empty() and not self._closed:
			try:
				...
				break
			if (conn.idle_duration > self._config.max_idle_time
	...
	async def _leak_detection_loop(self) -> None:
		while not self._closed:
			try:
				...
				async with self._lock:
					leaked = [
						c for c in self._checked_out
						if c.checkout_time and (now - c.checkout_time) > self._config.leak_detection_threshold
					]
				for conn in leaked:
					logger.warning(
