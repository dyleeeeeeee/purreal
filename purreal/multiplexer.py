...
from .types import (
	EventContext, PendingRequest, PhysicalState, PooledConnection,
	PoolEvent, SessionExpiredError, SessionState, SlotHealth,
	TeleportError,
)
...
class PhysicalSlot:
	...
	__slots__ = (
		"_pool", "_conn", "_state", "_queue", "_drain_task",
		"_health", "_id", "_closed",
	)
	...
	async def _apply_state(self, desired: PhysicalState) -> None:
		...
		needs_auth = (
			self._state is None
			or self._state.creds_hash != desired.creds_hash
		)
		needs_vars = (
			self._state is None
			or self._state.variables != desired.variables
		)
	...
	async def _handle_connection_failure(
		...
		logger.warning(f"{self._id}: connection failed, teleporting (attempt {request.retry_count})")
...
class VirtualSession:
	...
	__slots__ = (
		"_multiplexer", "_state", "_credentials", "_variables",
		"session_id", "_session_state", "_last_active",
	)
...
class SessionRouter:
	...
	def route(self, request: PendingRequest) -> PhysicalSlot:
		...
		for slot in self._slots:
			...
			elif state and state.namespace == request.state.namespace and state.database == request.state.database:
...
class SessionMultiplexer:
	...
	async def start(self) -> None:
		...
		logger.info(f"Multiplexer started: {self._num_slots} slots, max {self._max_sessions} sessions")
			...
			try:
				...
				for slot in self._slots:
					...
					if conn and (conn.expires_at - now) < 10.0:
						logger.info(f"{slot._id}: proactive migration (expires in {conn.expires_at - now:.1f}s)")
						await slot.proactive_migrate()
						await self._pool.events.emit(EventContext(
